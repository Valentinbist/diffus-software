...
The old command half of this module (`CrosspostingPublisher`, driving
`CreateDraft`/`PublishDraft`/`DiscardDraft`) is gone: the compose-a-post
wizard moved into crossposting itself (`/posts/new`, round 3), so this
context only ever reads posts now.
"""
